...
import { nativeToKrw } from '../song-pricing/utils/platform-exchange.util';
import { CreateSongRequestDto } from '../song-request/dto/request/create-request.dto';
import { OMAKASE_EVENTS } from './omakase.events';
import {
  FromChatOmakaseRequestDto,
  OmakaseLedgerResponseDto,
  OmakaseStatusDto,
...
export class OmakaseService {
  ...
  async getStatus(channelId: number): Promise<OmakaseStatusDto> {
    ...
    const updated = await this.prisma.$transaction(async (tx) => {
      ...
      for (const session of sessions) {
        await enqueuePlaybackRevision(tx, session.id, {
          commandType: 'omakase.settings_changed',
        });
      }
      return settings;
    });
  ...
  async setCount(params: {
    ...
    const status = await this.prisma.$transaction(async (tx) => {
      ...
      if (delta !== 0) {
        ...
        await enqueuePlaybackRevision(tx, params.liveSessionId, {
          commandType: 'omakase.count_changed',
        });
      }
  ...
  async consumeWithSongRequest(params: {
    ...
    const created = await this.songRequestService.createRequest(
      {
        ...params.request,
    ...
    const request =
      params.playNow === true
        ? await this.songRequestService.playNow((created as { id: number }).id)
        : created;
  ...
  private async emitUpdated(
    channelId: number,
    liveSessionId: number | null | undefined,
    status: OmakaseStatusDto,
  ): Promise<void> {
    const sessionIds =
      typeof liveSessionId === 'number'
        ? [liveSessionId]
        : (
    ...
    for (const sessionId of sessionIds) {
      this.eventEmitter.emit(OMAKASE_EVENTS.UPDATED, {
        channelId,
