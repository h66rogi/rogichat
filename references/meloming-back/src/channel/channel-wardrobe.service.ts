...
import { sanitizeSyncPostHtml } from '../sync-posts/utils/html-sanitizer';
import {
...
export class ChannelWardrobeService {
  ...
  async getPublicWardrobe(channelId: number): Promise<ChannelWardrobeResponseDto> {
  ...
  async getManageWardrobe(channelId: number): Promise<ChannelWardrobeResponseDto> {
    await this.ensureDefaultCategories(channelId);
  ...
  private async ensureDefaultCategories(channelId: number): Promise<void> {
    const categoryCount = await this.prisma.channelWardrobeCategory.count({
      where: { channelId },
    });
    if (categoryCount > 0) return;

    await this.prisma.channelWardrobeCategory.createMany({
      data: DEFAULT_CHANNEL_WARDROBE_CATEGORIES.map((category) => ({
        channelId,
        name: category.name,
  ...
  private async resolveCategory(channelId: number, categoryId?: number) {
    if (categoryId) {
  ...
  private async assertCategory(channelId: number, categoryId: number) {
    const category = await this.prisma.channelWardrobeCategory.findFirst({
  ...
  private async assertItem(channelId: number, itemId: number) {
