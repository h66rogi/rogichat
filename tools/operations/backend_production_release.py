...
def validate_archive(a, features=()):
    ...
    if a['execution_identity'] == 'config':
        require(all(a[r+'_execution_id'] == a[r+'_config_id'] for r in (('runtime','migration','decoder') if 'media' in features else ('runtime','migration'))))
    if 'media' in features:
        require(all(type(a[k]) is str and re.fullmatch(r'sha256:[a-f0-9]{64}', a[k])
...
def validate_request(v):
    ...
    require(type(v) is dict and fields <= set(v) <= fields | {'archive', 'features', 'decoder_image'})
    shared.validate_features(v)
    ...
    require(type(v['artifacts']) is dict and set(v['artifacts']) == set(shared.feature_artifacts(v, ARTIFACTS))
    ...
    if 'archive' in v:
        validate_archive(v['archive'], v.get('features', []))
...
def verify_qa_evidence(request, evidence):
    ...
    require(type(evidence) is dict and set(evidence) == (fields | ({'archive'} if 'archive' in request else set())
            | ({'features'} if 'features' in request else set()) | ({'decoder_image'} if 'decoder_image' in request else set())))
    for key in ('features', 'decoder_image'):
        if key in request:
...
def validate_running(item, role, request, push_required=False, desired=None):
    shared.validate_feature_running(item, role, desired)
    if role == 'decoder':
        require(desired is not None and item['Config']['Image'] == shared.execution_image(request,'decoder'))
        if 'archive' in request:
            require(item['Image'] == shared.execution_image(request,'decoder'))
        state = item['State']
    ...
    if role == 'api':
        ...
        if 'apple_auth' in request.get('features', []):
            require(env.get('AUTH_SECRET_FILE') == '/run/secrets/auth.json'
                    and mounts['/run/secrets/auth.json']['Source'] == str(AUTH)
                    and mounts['/run/secrets/auth.json']['RW'] is False)
        else:
            require('/run/secrets/auth.json' not in mounts and 'AUTH_SECRET_FILE' not in env)
        require(set(item['NetworkSettings']['Networks']) == {'rogichat-prod-app_jobs'})
    state = item['State']
    require(state['Status'] == 'running' and not state.get('OOMKilled') and not state.get('Error'))
    return state.get('Health',{}).get('Status') == 'healthy'
...
def wait_health(request):
    ...
    desired = shared.installed_compose(APP)
    shared.verify_decoder_volume(desired)
    deadline = time.monotonic()+90
    while time.monotonic() < deadline:
        healthy = True
        for role in (('decoder','api','worker') if 'media' in request.get('features', []) else ('api','worker')):
            item = inspect_container(role)
            if item is None or item['State']['Status'] == 'created':
                healthy = False
            else:
                healthy = validate_running(item,role,request,push_required,desired) and healthy
        if healthy:
            return
        time.sleep(2)
...
def stop_units():
    for role in shared.app_roles(APP):
        run(['/usr/bin/systemctl','stop','rogichat-prod-app@'+role],timeout=40)
        if role == 'decoder':
            run(['/usr/bin/systemctl','disable','rogichat-prod-app@decoder'])
...
def deploy(request, files, container):
    ...
    try:
        ...
        if UNIT.exists():
            old_roles = shared.app_roles(APP)
            stop_units()
            if 'decoder' in old_roles and 'media' not in request.get('features', []):
                shared.remove_stopped_decoder(inspect_container('decoder'), 'production')
        for role in (('decoder','api','worker') if 'media' in request.get('features', []) else ('api','worker')):
            item = inspect_container(role)
        ...
        docker(*compose_args(),'create','--force-recreate','--no-build','--pull','never',*shared.app_roles(APP),timeout=90)
        for role in shared.app_roles(APP):
...
def validate_templates(request, release, files, container):
    ...
    with tempfile.NamedTemporaryFile(prefix='rogichat-prod-compose-',dir='/var/tmp') as env:
        env.write((f'ROGICHAT_API_IMAGE={image}\nROGICHAT_WORKER_IMAGE={image}\n'
                   f"ROGICHAT_EDGE_NETWORK={request['edge_network']}\n").encode())
        env.flush()
        with tempfile.NamedTemporaryFile(prefix='rogichat-prod-compose-',dir='/var/tmp') as compose:
            compose.write(files['compose']); compose.flush()
            docker('compose','--env-file',env.name,'-f',compose.name,'config','--quiet')
...
def preflight():
    ...
    files = {k:protected(release/p) for k,p in shared.feature_artifacts(request, ARTIFACTS).items()}
    require(all(digest(files[k]) == request['artifacts'][k] for k in files))
    ...
    shared.verify_feature_secrets(request)
    files = shared.render_features(request, files)
