...
export const workflows = ['web.yml', 'backend.yml', 'security.yml', 'infrastructure.yml', 'mobile.yml'];
...
export async function verifyOnce({ sha, repository, token, cache, fetchImpl = fetch, stats }) {
  ...
  for (const workflow of workflows) {
    ...
    const proof = { workflow, id: run.id, attempt: run.run_attempt, url: exact.html_url, sha };
    cache.set(workflow, proof);
    evidence.push(proof);
  }
  if (evidence.length !== workflows.length) return null;
