...
The finite follow-up [run 35512987788](https://github.com/h66rogi/rogichat/actions/runs/35512987788) also passed at 13:17:43 UTC (22:17:43 KST), on harness `d6a949f` / executed merge `fb4d322924a736a2ab1a33657341c4070d74eb28` and the same runtime/build pins. It retained every preceding assertion and added forged release-signature rejection plus actual prepare interruption after a committed `QUARANTINED` checkpoint: the custodian held a real provider row lock, observed the durable checkpoint using READ COMMITTED, released MySQL custody and the row lock, observed prepare failure, then reacquired custody and resumed through a new protected invocation. The checkpoint epoch stayed unchanged and unrelated user generations did not increment twice. All 20 recorded rejection cases and the full joined flow passed in 17,843.07 ms (test process 18,627.12 ms); [checksummed follow-up measurements](evidence/m12/2026-09-20-joined-restore-resume.json) preserve the exact scope.
