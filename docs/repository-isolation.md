...
공개키 GitOps 예외도 승인됐다. 별도 관리 EC2를 사용자 승인 후 생성했다. Atlantis·배포 자동화 활성화는 아래 실행 경계 검증 후 진행한다.
...
관리 실행 위치는 별도 EC2로 정했다. 조직 read 상속, GitHub App·서버 인가기·격리 worker의 구현과 검증은 남아 있다.
