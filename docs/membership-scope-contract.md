...
policy_version, String(account.membership_generation), grants, revokedStickerIds]`.
...
existing rooms, while leaving M unchanged. Do not silently narrow this vector.
...
never an expected token, period, actor or hint. No schema/migration change.
