...
채널의 시각 기준은 2026-09-26 사용자가 첨부한 원본 화면으로 고정한다. 7월 카드형
개편판은 사용하지 않는다. 상세 원본 SHA·복사 범위·플랫폼 차이는
[재사용 조사 R86](mobile-reuse-audit.md#r86--사용자-원본-채널-화면-복원-2026-09-26)을 따른다.
사용자의 명시적 제한에 따라 로컬 Mac에서는 iOS·Android 에뮬레이터를 사용하지 않는다.
원격 CI의 에뮬레이터 검증은 허용된다.
