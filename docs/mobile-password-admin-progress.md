...
로그인 전 공통 안내에서 모든 Apple 계정에 SOOP 연결이 필수라는 단정을 제거한다.
실제 서버가 계정 연결을 요구하면 기존 연결 화면으로 안내하고, 독립 chat 권한을
받은 계정은 기존 capabilities 판정대로 진행한다. 멜로밍에서 이식한 화면 구조와
인증·권한 로직은 변경하지 않는 문구 수정이다. 고정된 QA19·QA20 산출물을 다시
빌드하거나 버전을 변경하지 않으며, 이 수정은 후속 앱 릴리스에 포함한다.
