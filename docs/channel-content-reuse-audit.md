...
> 2026-09-23 정정: 아래 표의 프런트엔드 이식 내용은 QA에서 전부 롤백했다. 현재 저장소에는 이 문서에 적힌 새 일정·옷장·노래책 프런트 화면이 없다. 백엔드와 스키마만 남아 있으며, 프런트는 멜로밍 원본 페이지의 코드와 기능을 그대로 옮기는 별도 작업으로 다시 진행한다.
