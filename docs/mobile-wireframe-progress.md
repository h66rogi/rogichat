...
| 알림 | device 등록/해제, routing, 권한 시점 | 알림 설정/딥링크 목적지 UI | push 계약과 계정 전환·권한 상실 시험 |
| 기기 QA | 테스트 기기에서 설치/실행 | 정적 빌드·상태 검사·패키지 격리 | 작은 화면/큰 글자/키보드/다크 모드/뒤로 동작 확인 |

다음 작업은 위 계약과 독립적인 **채팅 레이아웃 상세화(메시지 셀/입력바/스크롤),
연결·접근 제한 상태, 프로필 폼** 순서다. 입력/오류/접근성 규칙을 확정한 후 공통 fixture로
ViewModel을 연결한다. 이후 MB03 실제 인증→목록, MB04 영속 전송/동기화 순서를 따른다.
서버에서 막힌 항목은 이 표와 세부 계획에 남기고 다음 미연동 화면으로 이동한다.
