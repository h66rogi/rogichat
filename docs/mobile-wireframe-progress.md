...
> **이전 구현 기록 — 제품 방향 폐기.** 사용자는 출시용 MVP를 요구했으며 QA 미리보기 앱을
> 의도하지 않았다. 아래의 배포 앱 미리보기 허용과 개발 순서는 현재 실행 지시가 아니다.
> 최신 기준은 [통합 계획 §2.1·§8](mobile-implementation-plan.md)의 멜로밍 구현 재사용
> 제1원칙과 QA/prod 공통 제품 구성이다. 실제 후속 코드와 현재 블로커는
> [제품 구성 교체 기록](mobile-product-progress.md)을 따른다.
...
당시 “테스트 fixture만 사용” 경계를 QA 설치 앱의 오프라인 미리보기까지 확장한 것은
사용자 의도를 잘못 해석한 결정이었다. **이 허용은 폐기한다.** 당시 QA 포함/prod 제외 검사는
양 배포 산출물에서 fixture를 제외하는 검사로 교체해야 한다.
