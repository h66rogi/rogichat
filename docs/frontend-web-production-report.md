...
2026-09-20. Base `092c76b`; task branch `dokdo2013/web-production-service`.
...
- The pipeline owner confirmed the latest production build, artifact isolation and three negative runtime health cases (missing API origin, mismatched origin, invalid room UUID) pass. The first production profile-save/CSRF browser case passed after a native-fetch receiver correction. The preceding full run passed 76 cases with 2 platform skips; the remaining auth fixture race was made deterministic and chat regressions are owned by the chat worker. A final full browser rerun is in progress at this source commit; it is not reported as passed here.
