...
- Android 최신 lint에 맞춰 빈 `super.onCleared` 호출 제거, Modifier 인자 순서, 동일 위치의
  offset lambda와 `toUri` 확장만 조정했다. 원본 크기·간격·색상·애니메이션 값은 유지한다.
- 채널 설정의 visibility 보존, 선택 카테고리 전체 해제, nullable 노래/일정 PATCH를
  처리하는 기존 어댑터를 유지한다. 참조 저장소의 인증·환경·배포 설정은 가져오지 않는다.
- 사용자 지시에 따라 로컬 Mac에서는 iOS·Android 에뮬레이터를 사용하지 않는다. 원격 CI는
  허용된다. 로컬 기기용 컴파일과
...
| R69b | Rogichat QA24 `0188d93`의 `ConversationScreen`, `ConversationFeatureModel`, `MessageViewport` | **기존 스크롤 상태와 전송 흐름 수정**: 대화방 첫 진입은 이전 저장 위치보다 최신 메시지를 우선한다. iOS 18 `ScrollView`의 초기 하단 anchor와 화면 진입 이동을 함께 사용하고, 전송을 시작하면 기존 viewport의 최신 추적 상태를 활성화하여 대기 명령·확정 메시지가 입력창 바로 위로 이동하도록 한다. 과거 페이지 로딩 중의 현재 위치 보존은 유지한다. Meloming 채팅 UX는 이식하지 않는다. |
...
|---|---|---|---|
| R84 | R40·R42의 Android `RoomsViewModel` StateFlow와 iOS `RoomsScreenModel` Loadable, R44·R45의 선택 대상 확인창. 원본 SHA는 R39–R43과 같다. | 양 OS 방 목록·대화 화면, iOS `AppShell`, Android `AppNavigationBar` | **기존 구현 수정 재사용**: 확인된 참여 목록과 명령 대상·cycle 검증은 유지한다. 재조회 중 목록을 보존하고 갱신 문구를 행에서 제거한다. 사용자 제공 카카오톡 화면의 상단 검색·새 대화·설정, 가로 필터, 원형 이미지 영역과 전체 행 대화 열기, 목록 본문 구조를 실제 로기챗 기능에 맞춰 적용한다. 하단 탭바는 사용자 정정에 따라 기존 네이티브 구성을 복구했다. `나가기`는 목록에서 제거하고 대화 화면 메뉴에서 원래 참여 권한을 다시 확인한 뒤 실행한다. 실제 계약에 없는 광고·읽지 않음 수·마지막 메시지와 시간은 생성하지 않는다. 멜로밍 Talk/TalkV2 화면은 사용자 제외 조건이므로 재사용하지 않는다. |

## R85 — 채널 전체 삭제 후 원본 재복사 (2026-09-26)

R80–R84의 채널 구현과 검증 주장은 이번 이식의 기준으로 사용하지 않는다.
사용자 요청에 따라 iOS `Sources/Features/Channel`의 이전 22개 파일,
Android `feature/channel`의 이전 19개 파일을 모두 삭제했다. 그 뒤 아래 원본
디렉터리를 통째로 복사하고 **연결 수정 전 모든 파일의 바이트 일치**를 확인했다.

| 플랫폼 | 원본 SHA와 디렉터리 | 최초 복사 |
| --- | --- | --- |
| iOS | `18a33bbf96fe52b28d0de361916e20549bdcce6b`, `Meloming/Presentation/Channel` | 32 파일 |
| Android | `ecb3dbedb1dde5364bd617f072bc1ac4091b1a17`, `feature/channel/src/main/java/com/meloming/android/feature/channel` | 47 파일 |

파일별 원본 SHA-256, 현재 보존한 원본 파일의 경로, 새 연결 파일은
[소스 이식 기록](mobile-channel-source-import.json)에 있다. 최초 바이트 일치 기록은
최종 파일이 수정되지 않았다는 뜻이 아니다. 이후 수정은 다음과 같다.

- 후여르·선물하기·멤버십 제외. 기존 사용자 지시대로 Meloming Talk/TalkV2 채팅 화면은
  가져오지 않고 채널톡 버튼을 Rogichat의 기존 대화 화면으로 연결한다.
- 단일 후로기 채널의 `h66rogi` 식별자와 Rogichat API/웹 주소 사용. 현재 서버에서
  제공하지 않는 채널 디렉터리·홈·콘텐츠·클립·방명록·보이스커미션 진입은 제외한다.
- 원본의 프로필, 즐겨찾기, 검색, 카테고리/가수/난이도 필터, 정렬, 노래 행/상세,
  신청곡, 일정, 노래/일정 추가·수정, 채널 설정·카테고리 관리, 신청곡 콘솔을 복사한다.
  연결용으로 원본 모델·저장소·API·공통 컴포넌트·테마·가격 계산 코드도 함께 복사한다.
- Android는 Hilt 생성자 대신 기존 앱 그래프에서 원본 ViewModel을 생성한다.
  iOS는 Swift 6 격리 및 task-group 가변 캡처 제약을 수정한다.
- 기존 보호 자격증명 저장소로 인증한다. 익명 요청에는 인증 헤더를 붙이지 않는다.
  인증 요청에는 Bearer와 플랫폼 헤더를 함께 붙인다. 응답 전후 계정 변경을 검사하고
  계정 세대 변경 시 화면 상태도 새로 만든다. 리디렉션과 자동 쓰기 재전송을 사용하지 않는다.
- 백엔드 저장 계약에 맞춰 채널 공개 범위를 유지하고 링크 쓰기 필드를 제한한다.
...
- Kingfisher 7.12.0, Coil 3.0.4, Timber 5.0.1, Reorderable 2.4.3,
  ColorPicker Compose 1.1.2를 원본과 같은 버전으로 연결하고 라이선스를 포함한다.
  원본 직렬화 모델 실행에 필요한 Kotlin serialization 컴파일러 플러그인도 연결한다.
  Kingfisher의 실제 패키지 privacy manifest도 배포 검사 대상에 추가한다.

검증: 양 플랫폼 QA 컴파일, Android 원본 가격 계산 테스트와 네이티브 인증 연결 테스트,
의존성 고정/라이선스 검사. Simulator 직접 조작은 도구 권한에서 거절되어 이 기록에서
**실행 화면 대조 또는 픽셀 일치를 주장하지 않는다**. 소스 복사 사실과 실행 화면 검증을
구분한다. 공개 API는 실 QA 응답 8종을 복사한 Swift DTO로 디코딩해 확인했다. Android 단위 테스트 290개와 QA lint도 통과했다.
