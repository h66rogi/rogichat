...
후속 문서: [다각도 리뷰](backend-review.md), [기술 구현 계획](backend-implementation-plan.md),
[저비용 MVP 실행 단계](backend-mvp-execution-plan.md). 최신 사용자 결정은 MVP 실행 단계를 우선하며
과거 리뷰의 1,000명·Redis·다중 서버 운영 제안을 초기 출시 의무로 해석하지 않는다.

## 1. 확정 요구

- 첫 서비스는 후로기와 팬을 위한 방 하나. 처음부터 다중 사용자·다중 방·다중 스트리머를 지원하는 모델을 사용한다.
- 일반 단체방도 표현할 수 있는 채팅 코어 위에 버블형 방의 발송·열람 정책을 둔다.
- 초기 운영 규모는 1명 수준의 저비용 MVP. 다중 서버는 확장 가능한 구조와 테스트로 준비한다.
- 스트리머 전체 메시지, 팬의 비공개 메시지, 스트리머의 개별 답장, 메시지별 emoji 반응을 지원한다.
- 팬 메시지를 오른쪽에서 왼쪽으로 스와이프하면 바로 해당 팬의 개인답장 입력 상태로 전환한다.
- 스트리머는 자기 방에서 팬이 보낸 비공개 메시지를 선택해 별도 동의 없이 방 전체에 공개할 수 있다.
  팬 승인·공개 허용 플래그를 요구하지 않는다. 다른 방에 대한 권한은 생기지 않는다.
- 일반 어드민은 비공개 대화 열람 권한이 없다. 별도 범위·기간·사유가 있는 권한을 구분한다.
- MVP에서는 운영자 비공개 대화 예외 열람 기능도 비활성화한다.
- 과거 기록 공개 여부는 방별 설정. 입장 당시 정책·조회 시작점은 유지하고 재입장은 새로 계산한다.
- 사용자 PK는 UUID. 닉네임·프로필 이미지·선택 생일을 설정할 수 있다.
- 생일은 월·일만 선택 입력, 기본 비공개. 전역 공개 설정 하나가 본인이 참여한 모든 방의
  스트리머에게 적용된다. 이후 입장하는 방에도 적용하며 방별 override는 두지 않는다.
- 사진·동영상·이모티콘 등 다양한 메시지 콘텐츠를 지원한다.
- 팬과 스트리머 모두 미디어 전송을 허용하며 종류·용량·빈도는 방별로 제한한다.
- 작성자는 기간 제한 없이 자기 메시지를 모두에게 삭제할 수 있다.
- 채팅 기록은 삭제 요청 전까지 유지하며 경과 기간에 따른 자동 만료를 두지 않는다.
- 계정 탈퇴 시 본인 메시지·연결 공개본·첨부를 함께 삭제한다. 단순 방 퇴장은 삭제 요청이 아니다.
- 삭제 요청 후 신규 접근을 차단하고 서비스 관리 본문·파일은 24시간 이내 삭제한다.
  삭제 콘텐츠가 포함된 백업은 요청 시점부터 최대 30일 안에 만료·제거한다.
- 초기 스티커는 운영자가 등록·검수한 항목만 허용한다. 팬/스트리머 모두 전송할 수 있다.
...
MVP는 API 1개와 worker 1개를 같은 호스트에서 운영하고 Redis 없이 시작한다.
API가 socket-hint outbox만 소비해 로컬 소켓에 알리고 worker가 미디어·삭제·push 작업을 맡는다.
기본 제한은 DB 원자 counter/quota로 유지한다. 다중 API를 실제 운영할 때 공유 hint bus와
분산 limiter adapter를 검증한다. 알림 유실은 어느 배치에서도 주기적 DB sync로 복구한다.
...
공개본/첨부/quote는 표시 작성자뿐 아니라 원본 content owner/deletion root의
DELETING/DELETED 상태도 확인한다. 단순 계정 정지와 탈퇴 삭제를 혼동하지 않는다.
...
creator 자격·private 과거 열람 grant와 방장 공개 capability는 별개다. 정당한 방장의
팬 작성 비공개 메시지 공개는 `canPublishSource`로 검사하고 일반 열람 grant 부재로 축소하지 않는다.
방장 지정/이전은 강한 공개 권한 이동으로 감사하며 단순 스트리머 등록으로 부여하지 않는다.

### 전체공개와 삭제 연쇄

확정: 스트리머는 자기 방에서 팬이 보낸 비공개 메시지를 선택해 팬 동의 없이 공개할 수 있다.
스트리머가 작성한 개인답장은 팬 메시지 전체공개 명령의 원본이 아니다.
...
확정: `user_profiles.birthday_visible_to_streamers` 전역 boolean 하나를 사용한다. 기본 false이며
true이면 본인이 현재 참여한 모든 방과 이후 입장하는 방의 활성 스트리머에게 동일하게 적용한다.
공개를 끄면 모든 방에서 이후 생일 조회를 차단하고 profile privacy version을 갱신한다.
streamer별 profile revision 동기화·주기 재검증으로 변경을 전달하며 profile 전체 replacement로
생일이 제외된 응답의 기존 캐시도 제거한다. 다른 팬에게 변경 활동을 알리지 않는다.
열람자는 대상과 같은 방의 활성 스트리머여야 한다. 참여 관계 없는 다른 방 스트리머·팬·일반
어드민에게는 반환하지 않으며 전역 생일 검색/목록도 제공하지 않는다. 생일 미입력은 채팅을 제한하지 않는다.
...
FAN 방에서 스트리머가 답장 가능한 팬 메시지를 오른쪽에서 왼쪽으로 스와이프하면
추가 메뉴나 확인 단계 없이 해당 팬의 개인답장 입력 상태로 전환하고 입력창에 포커스를 준다.
스와이프 자체로 메시지를 전송하지는 않는다. 입력창에 개인답장 대상과 비공개 상태를 명확히
표시하고 사용자의 전송 동작으로 발송한다. 데스크톱·접근성을 위해 메뉴/키보드 대체 진입도 제안한다.

백엔드는 제스처가 아닌 명시적 private 발송 의도·대상 member·원본 메시지를 검증한다.
전송 시 방 참여·개별 답장 권한·원본 열람·대상 일치를 다시 확인한다. 대상 퇴장·권한 회수 등으로
전송할 수 없으면 오류를 반환하고 초안을 유지하며, 재시도를 포함해 전체 발송으로 자동 전환하지 않는다.
익명 공개본에서 다른 팬에게 원본 작성자 식별자를 제공하지 않는다. GROUP의 일반 인용 답장을
FAN 개인답장과 동일시하거나 팬 간 private 대화를 자동 허용하지 않는다.
...
확정: 채팅 기록은 삭제 요청 전까지 유지한다. 작성 후 30일/90일 등의 자동 만료나
오래된 메시지 일괄 삭제 정책을 두지 않는다. 유효한 메시지에 연결된 첨부도 같은 생명주기를
따르도록 설계하며, R2 lifecycle이 본문보다 먼저 정상 첨부를 삭제하지 않게 한다.
저장 기간과 열람 권한은 별개다. 퇴장·차단·재입장 및 입장 당시 조회 시작점 규칙은 그대로 적용한다.

삭제 요청에는 기존 모두에게 삭제 규칙을 적용해 원본·연결 공개본을 tombstone 처리하고
첨부 신규 조회를 차단한다. 나에게만 숨기기는 기록 삭제 요청으로 취급하지 않는다.
계정 탈퇴는 본인 메시지·연결 공개본·첨부 전체 삭제를 요청한다. 다른 사용자의 독립 메시지는
삭제하지 않으며 탈퇴자의 인용 본문·프로필 복사값은 반환하지 않는다.
서비스 관리 중인 본문·파일 삭제 기한은 요청부터 24시간, 해당 콘텐츠를 포함한 백업 제거의
상한은 요청부터 30일이다. 삭제 표식과 증거의 구체 수명은 실행 단계에서 최소화한다.
삭제 요청 전 보존을 삭제된 콘텐츠의 무기한 보관 근거로 쓰지 않는다. 복구는 삭제 원장을
재적용한 뒤에만 열며, R2 lifecycle 만료 설정만으로 24시간 삭제 완료를 판정하지 않는다.
미완료 업로드·임시 파일·캐시·전달 완료 outbox 등은 채팅 원본 보존과 구분해 별도 만료 정책을 둔다.
...
2. 인증된 backend ingress가 실제 bytes·동시성·timeout을 제한하며 bounded scratch에 받는다.
   전체 RAM buffering 없이 알려진 길이로 서버가 private quarantine에 PUT하고 임시본을 정리한다.
3. 완료 신고를 신뢰하지 않고 크기·magic bytes·디코딩·해상도·길이·checksum을 검사한다.
   이미지 EXIF/GPS 제거·재인코딩, 영상 변환·poster 생성, 악성 파일 검사를 worker에서 수행한다.
4. 검사한 bytes를 서버만 쓸 수 있는 새 attempt별 final key로 확정한다. 이전 작업이 격리 파일을
...
Content-Type만으로 내용 검증을 대신하지 않는다. 사용자용 presigned PUT/multipart는 확장 후속이다.
직접 PUT의 재사용·oversized 저장 비용을 intent 예약만으로 제한했다고 주장하지 않는다.
구체 ingress 크기/timeout/scratch 값은 MVP 실행 계획을 따른다.
SVG/HTML/실행 파일과 외부 URL import는 초기 미지원이다.
...
- 오른쪽 → 왼쪽 스와이프의 개인답장 진입, 명시적 전송, 대상 권한 상실 시 전체 발송 전환 금지.
- 오래된 정상 기록·첨부의 자동 삭제 방지, 보존과 열람권 분리, 삭제 요청 후 공개본·첨부 회수.
...
실행 상세: 미디어 크기/길이/빈도·운영 quota·삭제 표식 수명은 MVP 실행 단계의 초기값과 검증을 따른다.
생일 전역 공개·운영자 스티커 등록·탈퇴 삭제·24시간 삭제/30일 백업 상한·예외 열람 비활성화는 확정됐다.
메시지 편집·타인 읽음 표시·음성/검색/고정 메시지는 MVP 후속 범위로 분리한다.
채팅 기록의 삭제 요청 전 보존은 확정하며 bubble식 구독 글자 수 제한은 확정하지 않는다.
실제 사용자 데이터를 받기 전에 남은 삭제·탈퇴·복구 운영 정책을 고정한다.
