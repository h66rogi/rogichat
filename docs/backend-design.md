...
삭제된 본문·파일의 물리 삭제 완료 기한, 백업 만료 주기, 최소 삭제 표식 보존 기간과 탈퇴 시
삭제 범위는 별도 운영 정책으로 확정한다. 삭제 요청 전 보존을 삭제된 콘텐츠의 무기한 보관
근거로 사용하지 않으며, 백업 복구 시 삭제 요청을 재적용해 콘텐츠가 되살아나지 않게 한다.
...
구체적인 파일 크기/길이/빈도 한도, 스티커 등록 주체, 탈퇴 시 삭제 범위·물리 삭제 기한·백업 만료·삭제 표식 보존,
