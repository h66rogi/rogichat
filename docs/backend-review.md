...
후속 사용자 결정으로 초기 운영 규모/비용 기준이 변경되었다. 아래는 당시 리뷰 기록이며,
현재 실행 순서는 [저비용 MVP 계획](backend-mvp-execution-plan.md)과
[후속 3관점 리뷰](backend-mvp-review.md)가 우선한다. 다중 API·1,000명은 초기 출시 의무가 아니다.
