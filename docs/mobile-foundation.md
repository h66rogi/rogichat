...
배포 앱에서 제외한다. [제품 구성 교체 기록](mobile-product-progress.md)에 실제 코드 변경과
검증 범위, 아직 연결되지 않은 네이티브 인증·채팅 계약을 구분해 기록한다.
...
Gradle 9.8.0, Kotlin 2.4.20, Compose BOM 2026.09.00과 Xcode 26.6을 채택했다.
...
팬은 방장의 전체 메시지, 자신이 보낸 메시지와 자기에게 온 비공개 답장을 함께 본다.
팬의 새 일반 메시지는 방장 전용이고 스트리머의 새 일반 메시지는 참여 팬에게 보인다.
스트리머는 메시지를 선택한 경우에만 그 작성자에게 비공개 답장을 보낸다.
답장 버튼과 접근성을 먼저 확보하고 제스처는 로기챗 사용성 시험으로
결정한다. 스와이프를 채택할 경우 입력에만 진입하며 즉시 전송하지 않는다.
권한 상실 시 초안을 보존하고 전송을 거부하며 전체발송으로 바꾸지 않는다.

방 전체공개는 별도 명령이다. 공개본 작성자·원본 링크는 서버가 허용한 DTO만 표시한다. 키보드·스크롤 위치·
