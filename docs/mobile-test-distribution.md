...
저장소 검사, 임시 QA 키를 이용한 APK/AAB 빌드를 실행한다. iOS 입력은 한 macOS 작업에서
서명 안전장치를 확인하고, 상태 검사와 QA/prod 네 configuration 빌드를 별도 작업 디렉터리에서
병렬 실행한다. 두 검사 중 하나라도 실패하면 필수 결과가 실패한다. 공통 모바일
