...
이 문서는 최초 리뷰와 후속 재사용 통합 리뷰를 함께 보관한다. 아래 판정은 당시 계획의
리뷰 기록이다. 이후 사용자는 **멜로밍 구현의 최대 재사용을 제1원칙**으로 정하고 QA
미리보기 중심 제품 구성을 거부했다. 현재 실행 기준은 [통합 계획 §2.1·§8](mobile-implementation-plan.md)이다.
작은 단위 추출로 범위를 제한하거나 QA 합성 앱을 배포하는 근거로 이전 리뷰를 사용하지 않는다.
이번 사용자 보정에 대해 새로운 Sub Agent 리뷰를 받은 것으로 기록하지 않는다.
