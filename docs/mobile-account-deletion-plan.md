...
보호 기록의 보존과 화면 표시는 별도다. 사용자는 결과 화면의 확인 동작으로 표시를 닫을 수 있지만
ACK/UNKNOWN 기록은 지우지 않는다. 새 계정 B에서는 이전 A의 접수 번호·재인증 동작을 노출하지
않으며, 기기 정리 오류는 결과 화면을 닫아도 로그인 허용으로 바뀌지 않는다.

탈퇴뿐 아니라 이 흐름의 로컬 정보 초기화·재인증 확인창도 원래 계정/화면 세대를 보존한다.
옛 확인창이 새 계정의 credential·DB를 정리하거나 로그아웃시키면 안 된다. 서비스 진입이 지연된
사이 인증 callback이 복귀하는 경우도 실제 세션 소유권으로 차단한다. 로컬 초기화는 DB 정리에
성공한 뒤 보호 journal을 지우며, DB 실패를 기록 삭제 성공으로 대체하지 않는다.

서버 ACK 또는 결과 불명을 이미 관측한 뒤 로컬 정리·journal 조회가 실패해도 이를 미전송으로
바꾸지 않는다. 메모리의 관측 결과와 보호 기록을 일관되게 넘겨주며, 완료된 정리의 최종 읽기만
실패한 경우 로컬 재시도는 같은 작업의 메모리 상태만 정리한다. 새 계정이나 서버 요청을 건드리지
않는다. 실제 HTTP 전송 직전까지 원본 scope와 만료를 검사한다.

실행 증거는 저장소 종류를 구분한다. iOS host의 명시적 메모리 ByteStore 주입·실제 marker 파일·
GRDB on-disk 시험은 실제 iOS Keychain 실행 증거가 아니다. 임시 directory만 지정하고 실제 QA
Keychain service를 사용하는 검사는 하지 않는다. 실제 플랫폼 Keychain 검증은 격리된 namespace와
적절한 실행 환경을 준비한 뒤 별도 gate로 확인한다.
