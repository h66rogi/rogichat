...
variable "db_instance_class" {
  ...
  validation {
    condition     = var.db_instance_class == "db.r8g.large"
    error_message = "Changing the reviewed production database class requires a separate sizing and cost review."
  }
}
...
variable "aws_account_id" {
  type        = string
  sensitive   = true
  description = "Explicit approved production account; supplied only from private operations configuration."
  validation {
    condition     = can(regex("^[0-9]{12}$", var.aws_account_id))
    error_message = "Supply the explicitly reviewed AWS account."
  }
}
