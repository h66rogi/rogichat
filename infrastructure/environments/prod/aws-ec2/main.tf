...
# Dedicated production network; neither a shared VPC nor legacy infrastructure is mutated.
...
resource "aws_subnet" "db" {
  for_each                = { a = "10.79.11.0/24", c = "10.79.12.0/24" }
  vpc_id                  = aws_vpc.prod.id
...
resource "aws_security_group" "db" {
  ...
  description = "MySQL from the production app security group only"
  vpc_id      = aws_vpc.prod.id
}
...
output "database_admin_secret_arn" {
  value     = aws_rds_cluster.prod.master_user_secret[0].secret_arn
  sensitive = true
}
