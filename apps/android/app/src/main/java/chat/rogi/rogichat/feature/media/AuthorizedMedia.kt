...
@Composable
fun AuthorizedMedia(client: MediaClient, assetId: String, access: MediaAccess, modifier: Modifier = Modifier) {
    key(MediaPresentationIdentity(client.scope.presentationID, assetId, access)) {
        AuthorizedMediaBody(client, assetId, access, modifier)
    }
}

@Composable
private fun AuthorizedMediaBody(client: MediaClient, assetId: String, access: MediaAccess, modifier: Modifier) {
    ...
    LaunchedEffect(client, assetId, access, retry) {
        ...
        try {
            var lease = client.access(assetId, access)
            ...
            while (true) {
                ...
                if (lease.needsRenewal()) lease = client.renewAccess(assetId, access, lease)
            }
    ...
    Column(modifier) {
        ...
        } else if (file == null) { CircularProgressIndicator() }
