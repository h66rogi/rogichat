...
fun AccountDeletionStatus(state: DeletionState, onRetryCleanup: () -> Unit, onReset: () -> Unit, onAcknowledge: () -> Unit,
    ...
    val message = when {
        state.capacityReached -> "탈퇴 요청을 시작하지 못했어요. 잠시 후 다시 시도해 주세요."
        state.record == null -> "탈퇴 진행 상태를 확인하지 못했어요."
        state.busy -> "탈퇴 요청을 처리하고 있어요."
            ...
            DeletionPhase.PREPARING, DeletionPhase.CLEARED -> "탈퇴를 진행하지 못했어요. 다시 시도해 주세요."
            DeletionPhase.SENDING, DeletionPhase.UNKNOWN -> "탈퇴 요청을 확인하고 있어요. 잠시 후 다시 확인해 주세요."
        }
    }
    Surface(color = MaterialTheme.colorScheme.surfaceVariant) {
        Column(Modifier.fillMaxWidth().heightIn(max = 240.dp).verticalScroll(rememberScrollState()).padding(horizontal = 20.dp, vertical = 12.dp)) {
            Text("탈퇴 진행 상태", style = MaterialTheme.typography.labelMedium)
            Text(message, style = MaterialTheme.typography.bodyMedium)
            state.record?.receipt?.let { Text("접수 번호: ${it.requestId}", style = MaterialTheme.typography.bodySmall) }
            if (state.storageFailure) Text("이 기기의 정보를 정리하지 못했어요.", style = MaterialTheme.typography.bodySmall)
            if (state.busy) LinearProgressIndicator(Modifier.fillMaxWidth().padding(top = 8.dp))
                ...
                if (state.storageFailure || state.record?.cleanupPending == true)
                    TextButton(onClick = onRetryCleanup) { Text("다시 시도") }
                ...
                TextButton(onClick = onReset) { Text("이 기기에서 계정 정보 지우기") }
            }
        }
    }
}
