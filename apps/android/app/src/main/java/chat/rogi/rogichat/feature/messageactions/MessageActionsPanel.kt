...
fun MessageActionsPanel(token: ActionViewToken, record: ActionRecord?, busy: Boolean,
    reactions: MessageReactions?, onAction: (ActionViewToken, MessageAction, String?) -> Unit,
    ...
    val blocked = busy || record?.phase in setOf(ActionPhase.UNKNOWN, ActionPhase.PREPARING, ActionPhase.BLOCKED, ActionPhase.ACTOR_BLOCKED)
    Column {
        ...
        if (record?.phase != ActionPhase.BLOCKED) {
            Row {
                if (selected.hints.delete && !selected.anonymous) TextButton(enabled = !blocked,
                ...
                if (selected.hints.publish && !selected.anonymous && selected.contentKind in setOf("TEXT", "PHOTO"))
                    TextButton(enabled = !blocked, onClick = { confirmation = MessageAction.PUBLISH }) { Text("익명으로 공개") }
            }
            Row {
                reactionChoices.forEach { emoji -> TextButton(enabled = !blocked,
                    onClick = { onAction(token, MessageAction.SET_REACTION, emoji) }) { Text(emoji) } }
            }
            if (reactions?.mine != null) TextButton(enabled = !blocked,
...
@Composable
fun MessageModerationPanel(token: ActionViewToken, busy: Boolean,
    ...
    if (reporting) AlertDialog(onDismissRequest = { reporting = false }, title = { Text("신고 사유") },
        text = { Column { ReportReason.entries.forEach { reason ->
            TextButton(enabled = !busy, onClick = { reporting = false; onAction(token, MessageAction.REPORT, reason) }) { Text(reason.label) }
    ...
    if (blocking) AlertDialog(onDismissRequest = { blocking = false }, title = { Text("작성자 차단") },
        ...
        confirmButton = { TextButton(enabled = !busy, onClick = { blocking = false; onAction(token, MessageAction.BLOCK_ACTOR, null) }) { Text("차단") } },
