...
enum class AppTab(val label: String) { TALKS("대화"), CHANNEL("채널"), SETTINGS("설정") }
enum class AppPage(val title: String) {
    WELCOME("로기챗"), LINK("SOOP 계정 연결"), ROOMS("대화"), CHAT("대화방"),
    CHANNEL("채널"), SETTINGS("설정"), PROFILE("내 프로필"), ACCOUNT("계정 관리"), REPORT("신고 및 차단"),
...
data class ShellNavigation(
    ...
    val root: AppPage get() = if (tab == AppTab.SETTINGS) AppPage.SETTINGS else if (tab == AppTab.CHANNEL) AppPage.CHANNEL else when (access) {
        ShellAccess.SIGNED_OUT -> AppPage.WELCOME
        ShellAccess.LINK_REQUIRED -> AppPage.LINK
        ShellAccess.READY -> AppPage.ROOMS
        else -> AppPage.STATUS
    }
    val path: List<AppPage> get() = when (tab) { AppTab.TALKS -> talkPath; AppTab.CHANNEL -> emptyList(); AppTab.SETTINGS -> settingsPath }
    ...
    fun open(value: AppPage): ShellNavigation {
        ...
        return when (tab) { AppTab.TALKS -> copy(talkPath = talkPath + value); AppTab.SETTINGS -> copy(settingsPath = settingsPath + value); AppTab.CHANNEL -> this }
    }
    fun back(): ShellNavigation = when {
        ...
        tab == AppTab.CHANNEL -> selectTab(AppTab.TALKS)
