...
export async function installApi(page: Page, authenticated = false) {
  ...
  await page.route('https://api.qa.rogi.chat/v1/**', async route => {
    ...
    if (path === '/v1/auth/session') { await json(route, state.authenticated ? { authenticated: true, csrfToken: state.sessionToken, soopLinkStatus: 'VERIFIED' } : {}, state.authenticated ? state.sessionStatus : 401); return; }
    ...
    if (path === '/v1/rooms') { await json(route, { rooms: state.rooms ? [{ roomId: TEST_ROOM_ID, name: '후로기', mode: 'FAN', joined: state.joined, ...(state.joined ? { actorId: TEST_ACTOR_ID } : {}) }] : [], next: null }); return; }
    if (path === `/v1/rooms/${TEST_ROOM_ID}/join`) { expect(route.request().headers()['x-csrf-token']).toBe(state.sessionToken); state.joined = true; await json(route, { actorId: TEST_ACTOR_ID, historyPolicy: 'SINCE_JOIN', policyVersion: 1, visibleFromOrder: '1' }); return; }
