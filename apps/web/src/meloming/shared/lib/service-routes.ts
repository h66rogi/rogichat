...
export function getServiceLinks(): ServiceLink[] {
  return [
    ...
    { id: "notifications", label: "알림센터", href: getGlobalServiceHref("notifications") },
...
export function getRentalServiceUrl(): string {
  if (process.env.NEXT_PUBLIC_RENTAL_BASE_URL) {
    return process.env.NEXT_PUBLIC_RENTAL_BASE_URL.replace(/\/$/, "");
  }

  return isQaHost(
    typeof window !== "undefined" ? window.location.hostname : ""
  ) || isQaAppEnv()
    ? "https://rental.meloming.pri.sbalyd.com"
    : "https://rental.meloming.com";
}
