...
export async function resolveDefaultRoom(request: (path: string) => Promise<unknown>, configuredId: string | null): Promise<Room | null> {
  ...
  for (let pageNumber = 0; pageNumber < 100; pageNumber++) {
    ...
    for (const value of page.rooms) {
      ...
      uuid(room.roomId);
      if (typeof room.name !== 'string' || typeof room.mode !== 'string' || typeof room.joined !== 'boolean' || (room.isDefault !== undefined && typeof room.isDefault !== 'boolean') || (room.availability !== undefined && !['OWNER_PENDING', 'READY'].includes(room.availability))) throw new Error('INVALID_ROOM');
      if (configuredId ? room.roomId === configuredId : room.isDefault === true) {
