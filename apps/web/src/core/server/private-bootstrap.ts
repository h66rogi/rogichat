...
export type ServerPrivateBootstrap = { state: { kind: 'checking' } | { kind: 'ready'; session: Session; profile: Profile; generation: number } };
...
export async function loadPrivateBootstrap(): Promise<ServerPrivateBootstrap> {
  ...
  if (!credential) return { state: { kind: 'checking' } };
  ...
  try {
    ...
    if (!sessionAllowsChat(session)) return { state: { kind: 'checking' } };
