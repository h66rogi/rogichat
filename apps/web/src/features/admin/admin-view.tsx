...
function AdminAccount({ session, obscured }: { session: Session; obscured: boolean }) {
  ...
  if (room.kind !== 'ready') return <StatePanel title="채팅방을 확인할 수 없어요" retry={invalidateSession}>잠시 후 다시 시도해 주세요.</StatePanel>;
...
function RoomAccess({ session, roomId, roomName }: { session: Session; roomId: string; roomName: string }) {
  ...
  const failed = useCallback((failure: unknown) => {
    ...
    else setError(failure instanceof ApiError ? failure.message : '권한을 확인할 수 없어요. 다시 시도해 주세요.');
  }, []);
  useEffect(() => {
  ...
  return <section className="mx-auto flex max-w-xl flex-col gap-5 px-4 py-8"><h1 className="text-2xl font-semibold">관리자 페이지</h1><h2 className="text-lg font-medium">{roomName} · 임시 스트리머 권한</h2><p>현재 로그인한 내 계정에만 적용됩니다. 설정한 시간이 지나거나 회수하면 권한이 종료됩니다.</p>
    ...
    <Button variant="outline" disabled={busy} onClick={() => void operation(signal => load(signal))}>권한 새로고침</Button>
    ...
    <form className="flex flex-col gap-4" onSubmit={event => void grant(event)}>
      ...
      {uncertain && <p role="status">권한 발급 여부를 확인할 수 없어요. 다시 확인해 주세요.</p>}
      <Button type="submit" disabled={busy || (!uncertain && !canIssue)}>{busy ? '확인 중' : uncertain ? '발급 여부 확인' : '내 계정에 임시 권한 발급'}</Button>
    </form>
