...
export function LoginView({ reason }: { reason?: string | undefined }) {
  ...
  if (state.kind === 'unauthenticated') return <section className="flex flex-col gap-6"><h1 className="text-[28px] font-bold text-ink">로기챗 로그인</h1><p className="text-body">SOOP 계정으로 후로기의 채팅 공간에 참여하세요.</p>{reason && <p role="status" className="text-body">{reason}</p>}<SoopButton /></section>;
