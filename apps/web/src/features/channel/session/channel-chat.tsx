...
export function ChannelChat({ active, visit, initial }: { active: boolean; visit: number; initial: ChannelBootstrap | null }) {
  ...
  return <AuthorizedChat key={state.generation} active={active && !obscured} suspended={obscured} visit={visit} session={state.session} accountId={state.profile.id} scope={`${state.profile.id}:${state.generation}`} refresh={refresh} initial={initial} />;
}
