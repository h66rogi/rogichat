...
function AuthorizedChat({ session, scope, refresh }: { session: Session; scope: string; refresh: () => void }) {
  ...
  return <RealChatRoom session={session} key={scope + state.room.roomId} roomId={state.room.roomId} sessionScopeKey={scope} accountPartition={session.accountPartition} apiOrigin={api.origin} csrfToken={session.csrfToken} request={request} onInvalidate={refresh} />;
}
