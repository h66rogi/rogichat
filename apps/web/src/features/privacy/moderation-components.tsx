      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className={actionDialogContentClass} aria-busy={state === 'sending'}>
          ...
          {state === 'idle' ? <form className="mt-5 space-y-4" onSubmit={event => { event.preventDefault(); void report.current?.submit(props.scope.roomId, props.message.id, reason, detail); }}>
            ...
            <p className="text-xs leading-5 text-muted">민감한 개인정보는 적지 마세요. 상세 내용은 서버에서 최대 24시간 보관합니다.</p>
            ...
            {['unknown', 'missing', 'storageError'].includes(state) && <Button variant="outline" onClick={() => void report.current?.recover()}>신고 접수 확인</Button>}
            {['storageError', 'pendingExisting'].includes(state) && <a className="block text-sm underline" href="/settings">설정에서 신고 확인 기록 관리</a>}
            {canRetry && <Button variant="outline" onClick={() => void report.current?.retry(props.scope.roomId, props.message.id)}>동일한 신고 다시 제출</Button>}
