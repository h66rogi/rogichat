...
function MessageModerationForm(props: MessageModerationControlProps) {
  ...
  </>}{['unknown', 'missing'].includes(state) && <Button variant="outline" onClick={() => void report.current?.recover()}>신고 접수 확인</Button>}{canRetry && <Button variant="outline" onClick={() => void report.current?.retry(props.scope.roomId, props.message.id)}>동일한 신고 다시 제출</Button>}</div></details>
...
function BlockedActorsForm(props: BlockedActorsControlProps) {
  ...
  const load = async (after: string | null) => {
    if (loading) return; setLoading(true); setError(false); setSelected(null);
  ...
  return <section aria-label="차단한 사용자" className="space-y-3"><h2 className="font-semibold">차단한 사용자</h2><p>차단 목록에는 현재 닉네임이나 프로필을 추가로 조회하지 않습니다. 해제해도 퇴장·강퇴된 방의 참여 권한이 복구되지는 않습니다.</p><p role="status">{blockText[state]}</p><Button variant="outline" disabled={loading} onClick={() => void load(null)}>차단 목록 확인</Button>{error && <p role="alert">차단 목록을 확인하지 못했습니다. 다시 시도해 주세요.</p>}{page && <>{page.blocks.length === 0 && <p>이 페이지에 차단한 사용자가 없습니다.</p>}<ul>{page.blocks.map((row, index) => <li key={row.actorId} className="space-y-2 py-2"><span>차단 항목 {index + 1} · {new Date(row.blockedAt).toLocaleString('ko-KR')}</span><Button variant="outline" className="ml-2" onClick={() => setSelected(row.actorId)}>차단 항목 {index + 1} 해제</Button>{selected === row.actorId && <div><p>이 항목의 차단을 해제하시겠어요?</p><Button disabled={state === 'checking'} onClick={() => { setSelected(null); void flow.current?.change(props.roomId, row.actorId, false); }}>차단 해제 확인</Button><Button variant="outline" onClick={() => setSelected(null)}>취소</Button></div>}</li>)}</ul>{page.next && <Button disabled={loading} variant="outline" onClick={() => void load(page.next)}>다음 차단 목록</Button>}</>}</section>;
}
