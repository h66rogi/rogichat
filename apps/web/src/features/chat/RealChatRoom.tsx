...
function ScopedRealChatRoom({ roomId, apiOrigin, csrfToken, accountPartition, request, onInvalidate }: RealChatRoomProps) {
  ...
  useEffect(() => {
    const current = new ChatController(roomId, request, onInvalidate, csrfToken, accountPartition);
    let active = true;
...
function LiveRoom({ controller, connected }: { controller: ChatController; connected: boolean }) {
  ...
  return <div className="flex h-full min-h-0 flex-col"><section aria-label="전송 결과 확인" className="shrink-0">{state.commands.map((command, index) => <div key={command.id} className="flex flex-wrap items-center gap-2 px-4 py-2 text-sm"><span>결과 미확인 전송 {index + 1}</span><Button variant="outline" disabled={state.commandBusy} onClick={() => { void controller.reconcile(command.id); }}>결과 조회</Button>{command.canRetry && <Button variant="outline" disabled={state.commandBusy} onClick={() => { void controller.retry(command.id); }}>같은 전송 다시 시도</Button>}</div>)}</section><div className="min-h-0 flex-1"><ReactionContext.Provider value={{ controller, reactions: state.reactions, reactionRevision: state.reactionRevision }}><ChatRoomView
