...
test('membership history remains fixed until rejoin, owner/admin policy changes are audited and DTOs exclude private keys', { timeout: 20000 }, async t => {
  ...
  assert.equal(original.historyPolicy, 'SINCE_JOIN'); assert.ok(BigInt(original.visibleFromOrder) > 0n);
  assert.deepEqual(Object.keys(original).sort(), ['actorId', 'historyPolicy', 'policyVersion', 'visibleFromOrder']);
  ...
  assert.deepEqual(await f.join(f.fan1, room), original);
  assert.equal((await f.call(f.fan1, 'POST', `/rooms/${room.roomId}/leave`, {})).status, 204);
  const again = await f.join(f.fan1, room);
  assert.equal(again.actorId, original.actorId); assert.equal(again.historyPolicy, 'ALL_AVAILABLE'); assert.equal(again.visibleFromOrder, '0');
  ...
  assert.ok(BigInt(third.visibleFromOrder) > BigInt(original.visibleFromOrder));
  ...
  assert.deepEqual(Object.keys(item).sort(), ['actorId', 'joined', 'mode', 'name', 'roomId']);
  const events = await f.db.transactions.read(tx => tx.rows('SELECT action FROM audit_events WHERE room_id=?', [room.roomId]));
