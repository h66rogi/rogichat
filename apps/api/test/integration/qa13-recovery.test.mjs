...
test('recovery: real twelve-schema rejected, exact thirteen ready/no-op, partial/checksum drift/fourteen rejected by both entrypoints', { timeout: 180000 }, async t => {
  ...
  assert.equal(migrationManifest.length, 13);
  ...
  const directory = await mkdtemp(join(tmpdir(), 'rogichat-recovery-'));
  let created = false;
  t.after(async () => {
  ...
  for (const entry of migrationManifest.slice(0, 12)) {
  ...
  await migrate();
  const [twelve] = await admin.query('SELECT COUNT(*) AS n FROM _prisma_migrations'); assert.equal(twelve[0].n, 12);
  const [columns] = await admin.query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='login_transactions' AND COLUMN_NAME='channel'");
  assert.equal(columns.length, 0);
  await probe(false);
  const last = migrationManifest[12]; await mkdir(join(migrations, last.name));
  ...
  assert.deepEqual(after, before, 'exact13 migrate is a ledger no-op');
  assert.equal(await physical(), beforePhysical, 'exact13 migrate leaves physical schema unchanged');
...
test('product recovery composition has no native SOOP issuance and its OpenAPI makes no issuance promise', async t => {
  assert.equal(process.env.ROGICHAT_TEST_MYSQL, 'disposable');
  const database = new PrismaDatabase(readConfig('api'));
  const config = { audience: 'recovery-test', key: randomBytes(32), origin: 'http://localhost:3001', secure: false };
  ...
  for (const path of ['/v1/auth/native/soop/transactions', '/v1/auth/native/completions/exchange']) {
    ...
    assert.equal((await fetch(await app.getUrl() + path, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' })).status, 404);
  }
