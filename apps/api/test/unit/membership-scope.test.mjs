...
test('A retains the exact viewer ACL vector including account generation; other-room joins change A but not M', async () => {
  const member = { id: randomUUID(), room_id: roomId, role: 'MEMBER', active_period_id: periodId, acl_epoch: 3n, active_period: { visible_from_order: 9007199254740993n }, user: { membership_generation: 2n }, room: { mode: 'GROUP', policy_version: 4, content_epoch: 7n } };
  ...
  member.room.content_epoch++;
  const purged = await service.one({}, userId, roomId, now);
  assert.equal(two.membershipScope, purged.membershipScope); assert.notEqual(two.authorizationRevision, purged.authorizationRevision);
});
