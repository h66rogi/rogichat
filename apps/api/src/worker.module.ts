...
export class WorkerModule {
  ...
  static production(settings: RuntimeSettings): DynamicModule {
    ...
    return { module: WorkerModule, imports: [infrastructure, JobsModule.register(infrastructure, 'worker'), PublicationsCoreModule,
      ...
      { provide: WorkerLoop, inject: [Jobs, LifecycleState, Transactions, PublicationsCoreService, DATABASE, PushDeliveryService, NotificationFanoutService, ...(settings.media ? [MediaWorkerService] : [])],
        useFactory: (jobs: Jobs, lifecycle: LifecycleState, transactions: Transactions, publications: PublicationsCoreService, database: Database, delivery: PushDeliveryService, fanout: NotificationFanoutService, media?: MediaWorkerService) => new WorkerLoop(jobs, lifecycle,
          { PUBLICATION: lease => publications.publishText(transactions, lease), PUSH: lease => lease.roomId === null ? fanout.consume(lease) : delivery.consume(lease), ...(media ? { MEDIA: media.processMedia.bind(media) } : {}) },
