...
export class ModerationRepository {
  ...
  async ownBlocks(tx: Transaction, roomId: string, userId: string, after: string) {
    ...
    if (rows.length && member.status !== 'BANNED' && member.room.status === 'ACTIVE' && member.user.status === 'ACTIVE' && Boolean(await tx.prisma.users.findFirst({ where: { id: userId, ...chatUser(await tx.now()) }, select: { id: true } }))) {
  ...
  async ban(tx: Transaction, roomId: string, actorId: string, periodId: string | null, banned: boolean) {
    if (banned) {
      await tx.prisma.room_test_grants.updateMany({ where: { room_id: roomId, member_id: actorId, revoked_at: null }, data: { revoked_at: await tx.now() } });
