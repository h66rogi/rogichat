...
export const moderationDocs = {
  report: () => contract({ id: 'reportMessage', summary: '현재 열람 가능한 메시지 신고', auth: 'write', params: ['roomId', 'messageId'],
    ...
    description: '실제 저장된 접수증만 반환합니다. 동일 사용자·키·내용은 같은 접수증을 반환하며 다른 내용은 409입니다. 응답 유실 시 GET /v1/report-receipts/:idempotencyKey 로 복구합니다. 메시지 본문은 복사하지 않고 입력한 상세 내용은 최대 24시간 보관하며 삭제 작업에서도 제거합니다.' }),
  ...
  blocks: () => contract({ id: 'listActorBlocks', summary: '내 방별 차단 목록', params: ['roomId'], query: [afterQuery], response: object({ blocks: array(object({ actorId: uuid, blockedAt: timestamp })), next: nullable(uuid) }), description: '퇴장 후에도 자신의 차단 actorId를 복구할 수 있습니다. 다른 사용자의 차단이나 현재 프로필 정보는 반환하지 않습니다.' }),
