...
export class MediaService {
  ...
  async access(credentials: CommandCredentials, assetId: string, input: unknown) {
    const body = object(input, ['roomId', 'messageId', 'actorId', 'stickerId', 'variant']);
    if (typeof body.variant !== 'string') throw new ApiError('INVALID_REQUEST', 400);
    identifier(assetId);
    const context = { variant: body.variant, ...(body.roomId !== undefined ? { roomId: identifier(body.roomId) } : {}), ...(body.messageId !== undefined ? { messageId: identifier(body.messageId) } : {}), ...(body.actorId !== undefined ? { actorId: identifier(body.actorId) } : {}), ...(body.stickerId !== undefined ? { stickerId: identifier(body.stickerId) } : {}) };
