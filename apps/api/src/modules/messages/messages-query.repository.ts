...
import { fanSharedVisibleSql, publishedCopySql } from '../access/fan-message-policy.js';
...
async page(tx: Transaction, actorId: string, roomId: string, visibleFrom: string, window: MessageWindow, limit: number, now: Date) {
  ...
  const rows = await tx.rows<RowDataPacket>(`SELECT m.id,m.version,m.created_order,m.created_at,m.deletion_root_id,m.sender_member_id,m.text_content,m.content_kind,s.kind,r.mode AS room_mode,r.owner_member_id,${publishedCopySql('m')} AS published_active,p.nickname,av.id AS avatar_id,${blocked} AS blocked,
    ...
    JOIN message_streams s ON s.id=m.stream_id AND s.room_id=m.room_id JOIN rooms r ON r.id=m.room_id JOIN room_members viewer ON viewer.id=? AND viewer.room_id=m.room_id JOIN users u ON u.id=m.content_owner_user_id
...
async stickerRevocations(tx: Transaction, roomId: string, actorId: string, visibleFrom: string, delegated = false) {
  ...
  const room = await tx.prisma.rooms.findUnique({ where: { id: roomId }, select: { mode: true, owner_member_id: true } });
  if (!room) return [];
  ...
  return tx.prisma.sticker_catalog.findMany({ where: { status: 'REVOKED', messages: { some: { room_id: roomId, message: {
    sender_member_id: { notIn: blocks.map(row => row.target_actor_id) }, created_order: { gte: BigInt(visibleFrom) }, OR: [
      { stream: { kind: 'ROOM_SHARED' }, ...(room.mode === 'FAN' && room.owner_member_id !== actorId ? { OR: [
        { deletion_root_id: { not: null }, published_as: { some: { state: 'PUBLISHED' } } },
        { sender_member_id: actorId }, ...(room.owner_member_id ? [{ deletion_root_id: null, sender_member_id: room.owner_member_id }] : []),
      ] } : {}) },
      { stream: { kind: 'RESTRICTED', ...(delegated ? {} : { grants: { some: {
