...
export function sendInput(body: unknown): SendInput {
  ...
  const content = object(input.content, ['type', 'text', 'assetIds']);
  let parsed: SendInput['content'];
  if (content.type === 'TEXT') {
    if (typeof content.text !== 'string' || content.assetIds !== undefined) throw new ApiError('INVALID_REQUEST', 400);
  ...
  } else if (content.type === 'PHOTO' || content.type === 'VIDEO' || content.type === 'STICKER') {
    if (content.text !== undefined || !Array.isArray(content.assetIds) || content.assetIds.length < 1 || content.assetIds.length > (content.type === 'PHOTO' ? 4 : 1)) throw new ApiError('INVALID_REQUEST', 400);
