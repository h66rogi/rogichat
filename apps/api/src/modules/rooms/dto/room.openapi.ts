...
export const roomDocs = {
  list: () => contract({ id: 'listRooms', summary: '입장 가능한 방 조회', query: [afterQuery], response: object({ rooms: array({ oneOf: [object({ roomId: uuid, name: text, mode: enumeration('FAN', 'GROUP'), joined: { type: 'boolean', enum: [true] }, actorId: uuid, ...scopeFields }), object({ roomId: uuid, name: text, mode: enumeration('FAN', 'GROUP'), joined: { type: 'boolean', enum: [false] } })] }), next: nullable(uuid) }) }),
  privateRecipients: () => contract({ id: 'listPrivateRecipients', summary: '비공개 메시지 수신자 조회', params: ['roomId'], query: [afterQuery], response: object({ recipients: array(object({ actorId: uuid, nickname: text, avatar: nullable(object({ assetId: uuid })) })), next: nullable(uuid) }), description: 'SOOP 연동된 활성 FAN 방 구성원만 조회합니다. 팬은 스트리머, 스트리머는 팬 수신자만 조회합니다. 현재 멤버십과 대화 권한을 검사한 결과를 최대 50개 반환합니다. GROUP 방은 403이며, 관리자 권한만으로 조회할 수 없습니다. 목록은 전송 권한의 영구 보장이 아니며 실제 발송 시 재검사합니다.' }),
