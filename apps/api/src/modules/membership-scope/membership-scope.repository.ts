...
export class MembershipScopeRepository {
  async batch(tx: Transaction, userId: string, roomIds: string[], now: Date) {
    const members = await tx.prisma.room_members.findMany({ where: { user_id: userId, room_id: { in: roomIds }, status: 'ACTIVE', room: { status: 'ACTIVE' }, active_period: { is: { left_at: null } } }, take: maximum + 1,
      select: { id: true, room_id: true, role: true, active_period_id: true, acl_epoch: true, active_period: { select: { visible_from_order: true } }, user: { select: { membership_generation: true } }, room: { select: { mode: true, policy_version: true } } } });
    ...
    const revoked = await tx.rows<{ room_id: string; id: string }>(`SELECT DISTINCT m.room_id,sc.id FROM sticker_catalog sc JOIN message_stickers ms ON ms.sticker_id=sc.id JOIN messages m ON m.id=ms.message_id AND m.room_id=ms.room_id JOIN message_streams s ON s.id=m.stream_id AND s.room_id=m.room_id JOIN room_members rm ON rm.room_id=m.room_id AND rm.user_id=? JOIN membership_periods p ON p.id=rm.active_period_id AND p.member_id=rm.id AND p.room_id=rm.room_id WHERE sc.status='REVOKED' AND m.room_id IN (${roomIds.map(() => '?').join(',')}) AND m.created_order>=p.visible_from_order AND (s.kind='ROOM_SHARED' OR (s.kind='RESTRICTED' AND EXISTS (SELECT 1 FROM stream_grants g WHERE g.stream_id=s.id AND g.member_id=rm.id AND g.can_read=1 AND g.revoked_at IS NULL AND g.valid_from<=? AND (g.expires_at IS NULL OR g.expires_at>?)))) ORDER BY m.room_id,sc.id LIMIT 10001`, [userId, ...roomIds, now, now]);
    if (members.length > maximum || grants.length > maximum || revoked.length > maximum) throw new ServiceUnavailableException();
    return { members, grants, revoked };
  }
}
