  ...
  async batch(tx: Transaction, userId: string, roomIds: string[], now: Date) {
    ...
    return new Map(members.map(m => [m.room_id, {
      ...
      authorizationRevision: createHmac('sha256', this.config.key).update('authorization-revision:v1:').update(JSON.stringify([this.config.audience,
        [m.id, m.role, m.room.mode, m.active_period_id, String(m.active_period!.visible_from_order), String(m.acl_epoch), m.room.policy_version, String(m.user.membership_generation), byMember.get(m.id) ?? [], byRoom.get(m.room_id) ?? []]])).digest('base64url'),
    }]));
  }
}
