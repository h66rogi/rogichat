...
function channel(value: string): void {
  if (!isChannelIdentifier(value, true)) throw new ApiError('NOT_FOUND', 404);
}
...
export class MelomingArtistsController {
  constructor(@Inject(SongbookService) private readonly songs: SongbookService,
    @Inject(MelomingArtistService) private readonly artists: MelomingArtistService,
    @Inject(AUTH_CONFIG) private readonly config: AuthConfig) {}
  ...
  @Post('channel/:channelId/artists') @channelDoc('melomingArtistCreate', '원본 가수 등록', 'write', 201)
  create(@Param('channelId') channelId: string, @Req() request: Request) {
    channel(channelId);
    return this.artists.create(readCommandCredentials(request, this.config), request.body);
  }

  @Put('channel/:channelId/artists/:artistId') @channelDoc('melomingArtistUpdate', '원본 가수 수정', 'write')
  update(@Param('channelId') channelId: string, @Param('artistId') artistId: string, @Req() request: Request) {
    channel(channelId);
    return this.artists.update(readCommandCredentials(request, this.config), id(artistId), request.body);
  }

  @Delete('channel/:channelId/artists/:artistId') @channelDoc('melomingArtistDelete', '원본 가수 삭제', 'write')
  remove(@Param('channelId') channelId: string, @Param('artistId') artistId: string, @Req() request: Request) {
    channel(channelId);
    return this.artists.remove(readCommandCredentials(request, this.config), id(artistId));
  }
}
