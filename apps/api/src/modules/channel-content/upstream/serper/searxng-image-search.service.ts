...
import { SearchCache, SearchEnvironment } from './search-cache.js';
import type { SerperImageItemDto, SerperResponseDto } from './serper.response.dto.js';
...
export class SearxngImageSearchService {
  ...
  constructor(
    ...
    private readonly configService: SearchEnvironment,
  ) {}
  ...
  async searchImages(dto: SpotifySearchDto): Promise<SerperResponseDto> {
    ...
    if (!title || !artist) {
      throw new BadRequestException('노래 제목과 가수명이 필요합니다.');
    }
