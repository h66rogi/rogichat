...
import { songAutocompleteCacheKeys } from './song-autocomplete.cache-keys.js';

type CacheScope = {kind:'channel';channelId:number}|{kind:'global'};
type SongAutocompleteItemDto = {id:number;title:string;artistId:number;artistName:string};
type SongAutocompleteResponseDto = {suggestions:SongAutocompleteItemDto[]};
type SongArtistSuggestionDto = {artistId:number|null;artistName:string;isExisting:boolean;matchCount?:number};
type SongArtistSuggestResponseDto = {suggestions:SongArtistSuggestionDto[];canCreateNew:boolean};
...
export class SongAutocompleteService {
  ...
  // The source cache is omitted until Rogichat has a channel-scoped invalidation service.
  private readonly cacheTracker = {
    get: async <T>(key:string):Promise<T|null> => { void key; return null; },
    trackAndSet: async (key:string,value:unknown,ttl:number,scope:CacheScope):Promise<void> => {
      void key; void value; void ttl; void scope;
    },
  };
  ...
  private async fetchPopularSongs(
    ...
    if (channelId !== undefined && channelId !== 1) return [];
    const {roomId} = await this.transactions.read(tx=>this.repository.primary(tx));
    ...
    const channelClause = Prisma.sql`AND s.channel_id = ${roomId}`;

    const rows = await this.transactions.read(tx=>tx.prisma.$queryRaw<PopularSongRow[]>(Prisma.sql`
      SELECT
        s.id AS id,
        s.title AS title,
  ...
  private async fetchArtistsByExactTitle(
    ...
    const rows = await this.transactions.read(tx=>tx.prisma.$queryRaw<ArtistMatchRow[]>(Prisma.sql`
      SELECT
        s.artist_id AS artistId,
        a.name AS artistName,
        COUNT(*) AS matchCount
