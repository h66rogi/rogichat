...
export class MelomingLiveSongRequestController {
  ...
  create(@Req() request:Request) { return this.requests.create(readCommandCredentials(request,this.config),request.body); }
  ...
  playNext(@Req() request:Request) { return this.requests.advance(readCommandCredentials(request,this.config),request.query as Record<string,unknown>,'next'); }
  @Post('skip-current') @channelDoc('melomingLiveSkipCurrent','원본 현재 신청곡 스킵','write',201)
  skipCurrent(@Req() request:Request) { return this.requests.advance(readCommandCredentials(request,this.config),request.query as Record<string,unknown>,'skip'); }
  @Delete('queue') @channelDoc('melomingLiveQueueClear','원본 신청곡 대기열 초기화','write')
  clear(@Req() request:Request) { return this.requests.clear(readCommandCredentials(request,this.config),request.query as Record<string,unknown>); }

  @Post(':id/play-now') @channelDoc('melomingLivePlayNow','원본 신청곡 즉시 재생','write',201)
  playNow(@Param('id') requestId:string,@Req() request:Request) { return this.requests.playNow(readCommandCredentials(request,this.config),id(requestId)); }
  @Patch(':id/status') @channelDoc('melomingLiveRequestStatus','원본 신청곡 상태 변경','write')
  status(@Param('id') requestId:string,@Req() request:Request) { return this.requests.status(readCommandCredentials(request,this.config),id(requestId),request.body); }
  @Patch(':id/order') @channelDoc('melomingLiveRequestOrder','원본 신청곡 순서 변경','write')
  order(@Param('id') requestId:string,@Req() request:Request) { return this.requests.order(readCommandCredentials(request,this.config),id(requestId),request.body); }
  @Delete(':id/mine') @channelDoc('melomingLiveRequestCancel','원본 내 신청곡 취소','write')
  mine(@Param('id') requestId:string,@Req() request:Request) { return this.requests.remove(readCommandCredentials(request,this.config),id(requestId),true); }
  @Delete(':id') @channelDoc('melomingLiveRequestDelete','원본 신청곡 삭제','write')
  remove(@Param('id') requestId:string,@Req() request:Request) { return this.requests.remove(readCommandCredentials(request,this.config),id(requestId)); }
}
...
export class MelomingManualSongRequestController {
  ...
  manual(@Param('id') sessionId:string,@Req() request:Request) {
    return this.requests.manual(readCommandCredentials(request,this.config),id(sessionId),request.body);
  }
}
