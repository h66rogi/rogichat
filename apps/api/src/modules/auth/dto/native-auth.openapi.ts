...
export const nativeStartRequest = { oneOf: [
  object({ ...startFields, intent: enumeration('login'), termsVersion: enumeration('2026-09-20') }),
...
export const nativeAuthDocs = {
  start: () => applyDecorators(contract({ id: 'startNativeSoopAuthentication', summary: '네이티브 SOOP 인증 트랜잭션 시작', auth: 'none', security,
    ...
    errors: [400, 401, 403, 413, 429], description: admission + '앱이 생성한 S256 challenge와 returnState를 바인딩합니다. login은 명시적 약관 동의가 필요하며 link는 termsVersion을 받지 않습니다. 전체 트랜잭션은 600초, 반환된 일회성 launch URL은 60초입니다. 앱이 시스템 브라우저로 URL을 열어야 하며 URL·코드·토큰은 로그에 남기지 않습니다. broker 미설정은 503이며 세션 쿠키를 발급하지 않습니다.' }), nativeAdmission()),
