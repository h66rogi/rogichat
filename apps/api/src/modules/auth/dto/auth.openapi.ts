...
import { afterQuery, boolean, contract, empty, enumeration, nullable, object, text, uuid } from '../../../common/openapi/schema.js';
...
const activeSession = object({ id: uuid, kind: enumeration('web', 'ios', 'android', 'other'), createdAt: { ...text, format: 'date-time' }, expiresAt: { ...text, format: 'date-time' }, current: boolean });
export const authDocs = {
  ...
  sessions: () => contract({ id: 'listSessions', summary: '로그인된 기기 조회', query: [afterQuery], response: object({ sessions: { type: 'array', items: activeSession }, next: nullable(uuid) }), errors: [400, 401] }),
  revokeSession: () => contract({ id: 'revokeSession', summary: '로그인된 기기 해제', auth: 'write', params: ['id'], body: empty, status: 204, errors: [400, 401, 403, 404] }),
