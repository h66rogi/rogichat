...
export type ErrorCode = 'UNAUTHENTICATED' | 'FORBIDDEN' | 'SOOP_LINK_REQUIRED' | 'INVALID_REQUEST' | 'INVALID_CURSOR' | 'AUTH_FAILED' | 'AUTH_UNAVAILABLE' | 'RATE_LIMITED' | 'CONFLICT' | 'NOT_FOUND' |
  'MEMBERSHIP_SCOPE_MISMATCH' | 'TERMS_REQUIRED' | 'RECENT_AUTH_REQUIRED' | 'LINK_SESSION_CHANGED' | 'SOOP_LINK_CONFLICT' | 'APPLE_LINK_CONFLICT' | 'NATIVE_CALLBACK_FAILED' |
  'MEDIA_CAPACITY' | 'MEDIA_STATE' | 'MEDIA_UPLOAD_FAILED' | 'MEDIA_UNAVAILABLE' | 'INVALID_MEDIA_INTENT' | 'MEDIA_FORBIDDEN' | 'MEDIA_SIGNATURE_MISMATCH' | 'INVALID_MEDIA_METADATA';
