...
export class AuthFlow {
  ...
  async start(intent: 'login' | 'link', browser: string, token?: string, csrf?: string): Promise<{ url: string; state: string }> {
    ...
    await this.transactions.write(async tx => {
      ...
      if (principal) {
        const recent = await this.repository.recentSession(tx, principal.sessionId);
        if (!recent) throw new ApiError('FORBIDDEN', 403);
      }
      await this.repository.insert(tx, { id, stateDigest: digest(state), browserDigest: digest(browser), verifier: encrypt(verifier, this.config.key), intent, audience: this.config.audience, userId: principal?.userId, sessionId: principal?.sessionId });
    });
    try {
