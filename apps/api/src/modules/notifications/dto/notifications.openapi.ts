...
export const notificationsOpenApi = {
  ...
  paths: {
    ...
    '/v1/me/notification-preferences': {
      ...
      put: { operationId: 'putNotificationPreferences', description: 'Change own account preference with current GET generation as expectedGeneration; stale writes return CONFLICT. Changes increment its generation. Native enable requires verified SOOP/current terms, a configured provider and a current native subscription bound to this session and account generation; missing or stale binding returns CONFLICT. Clients obtain OS permission and register before explicit enable. Unconfigured providers return AUTH_UNAVAILABLE; disabling always remains available.', requestBody: body('NotificationPreferencesInput'), responses: { '200': response('NotificationPreferences'), ...errors, '409': { description: 'CONFLICT: stale or exhausted generation' }, '503': { description: 'AUTH_UNAVAILABLE' } } },
    },
