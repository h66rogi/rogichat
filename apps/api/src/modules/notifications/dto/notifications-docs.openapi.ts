...
export const notificationsDocs = {
  ...
  setPreferences: () => contract({ id: 'putNotificationPreferences', summary: 'Change own notification preferences', auth: 'write', body: notificationPreferencesInput, response: notificationPreferences,
    description: 'Supply current GET generation as required expectedGeneration; stale or exhausted generation returns 409 CONFLICT. Changes increment generation. Native enable requires verified SOOP/current terms, a configured provider and a current native subscription bound to this session and account generation (missing/stale binding is 409). Clients must obtain actual OS permission and register before explicit enable; registration never enables preferences. Provider unavailable returns 503 for web or native. Disable remains available without Push configuration.', errors: [400, 401, 403, 409] }),
