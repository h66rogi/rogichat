...
export function createOpenApiDocument(app: NestExpressApplication, auth?: AuthConfig): OpenAPIObject {
  const config = new DocumentBuilder().setTitle('로기챗 API').setVersion('1.0')
    ...
    .addServer('/')
    .addBearerAuth({ type: 'http', scheme: 'bearer', bearerFormat: 'opaque 43-character base64url', description: '네이티브 토큰과 X-Rogi-Client를 함께 사용합니다. 웹 세션 쿠키 및 CSRF와 혼용하면 400입니다. native SOOP 트랜잭션의 일회성 완료 코드 교환으로 발급하며 broker 미설정은 503입니다. refresh endpoint는 없습니다.' }, 'nativeBearer')
