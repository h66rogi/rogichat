...
export function readRuntimeSettings(role: Role): RuntimeSettings {
  ...
  return { config, push: readPushConfig(), ...(deletion ? { deletion: { config: deletion } } : {}), ...(auth ? { auth } : {}), ...(enabled && media ? { media: { config: media, scratch: process.env.MEDIA_SCRATCH_DIR!, ...(role === 'worker' ? { decoderSocket: process.env.MEDIA_DECODER_SOCKET! } : {}) } } : {}) };
}
