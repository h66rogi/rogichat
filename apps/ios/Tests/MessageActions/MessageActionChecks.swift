...
@main struct MessageActionChecks {
    ...
    @MainActor static func main() throws {
        ...
        var entering = MessageViewport()
        check(entering.initialize(restored: nil) == .latest)
        entering.observed(anchor: anchor, atLatest: false)
        check(entering.showLatest() == .latest)
        check(try entering.incomingCommitted([d]) == .latest)
        check(entering.incomingCount == 0)
