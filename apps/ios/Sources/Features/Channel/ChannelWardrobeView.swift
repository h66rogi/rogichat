...
// Copied from meloming-ios 18a33bb: ChannelNativeHubViews.swift wardrobe section.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if viewModel.isLoading && viewModel.items.isEmpty {
                ...
                ContentUnavailableView("옷장을 불러올 수 없어요", systemImage: "exclamationmark.triangle", description: Text(errorMessage))
                .padding(.top, 48)
            } else if visibleItems.isEmpty {
                ContentUnavailableView("등록된 옷장이 없어요", systemImage: "tshirt", description: Text("새로운 의상과 이미지가 추가되면 여기에 모아둘게요."))
                .padding(.top, 48)
            } else {
...
private struct ChannelWardrobeGridItem: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            AsyncImage(url: ChannelImageURL.resolve(item.imageURL)) { image in
                image.resizable().scaledToFill()
            } placeholder: {
                Rectangle().fill(Color(.secondarySystemBackground)).overlay { ProgressView() }
            }
                .frame(maxWidth: .infinity)
