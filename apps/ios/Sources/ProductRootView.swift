        ...
        case .account:
            if let account = session.account {
                ...
                AccountScreen(account: account, capabilities: session.capabilities,
                              onLink: { navigation.selectTab(.talks) }, onLinkApple: { Task { await session.linkApple() } }, onSignOut: { try await session.signOut() }, prepareDeletion: { session.deletionIntent(expectedGeneration: generation) }, onDelete: { session.startDeletion($0) })
                    .id(session.generation)
            }
        case .link:
